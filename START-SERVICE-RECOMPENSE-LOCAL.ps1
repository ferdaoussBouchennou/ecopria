# Script pour démarrer le Service-Recompense en LOCAL (pas Docker)

$ErrorActionPreference = "Stop"

Write-Host "`n========================================" -ForegroundColor Cyan
Write-Host "  DÉMARRAGE SERVICE-RECOMPENSE (LOCAL)" -ForegroundColor Cyan
Write-Host "========================================`n" -ForegroundColor Cyan

$SERVICE_DIR = "backend\service-recompense"
...
Write-Host "`n[2/4] Vérification de MySQL (port 3311)..." -ForegroundColor Yellow
try {
    $mysqlTest = & mysql -h localhost -P 3311 -u ecopria -pecopria_pass_2026 -e "SELECT 1" 2>&1
    if ($LASTEXITCODE -ne 0) {
        throw "MySQL non accessible"
    }
    Write-Host "  MySQL OK" -ForegroundColor Green
} catch {
    Write-Host "  ERREUR: MySQL non accessible sur le port 3311" -ForegroundColor Red
...
# Démarrer le service
Write-Host "`n[4/4] Démarrage du service sur le port 9093..." -ForegroundColor Yellow
